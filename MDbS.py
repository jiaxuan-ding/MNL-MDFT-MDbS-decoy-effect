...
def cal_accumu_p(tdata,beta):
    sc=beta[0]
    beta_0=0.7/(1+np.exp(beta[1]))
    beta_1=-110/(1+np.exp(beta[2]))
    ...
    matrix_g=np.full([tdata.shape[0],tdata.shape[1],tdata.shape[2]*tdata.shape[3],tdata.shape[2]*tdata.shape[3]], np.nan) #matrix g
    
    for i in range(0,tdata.shape[3]):
        temp=((np.repeat(tdata[:,:,:,i],tdata.shape[2])).reshape(tdata.shape[0],tdata.shape[1],tdata.shape[2],tdata.shape[2])-(np.tile(tdata[:,:,:,i],tdata.shape[2])).reshape(tdata.shape[0],tdata.shape[1],tdata.shape[2],tdata.shape[2]))>0
        z=beta_1*(((output[:,:,tdata.shape[2]*i:tdata.shape[2]*i+tdata.shape[2],tdata.shape[2]*i:tdata.shape[2]*i+tdata.shape[2]])[temp])-beta_0)
        (matrix_g[:,:,tdata.shape[2]*i:tdata.shape[2]*i+tdata.shape[2],tdata.shape[2]*i:tdata.shape[2]*i+tdata.shape[2]])[temp]=1/(1+np.exp(z))
...
def trans(params,cov):
    ...
    beta_0=0.7/(1+np.exp(sample_p[:,1]))
    beta_1=-110/(1+np.exp(sample_p[:,2]))
    return np.std(beta_0),np.std(beta_1)

#defining the callback fuction of the estimation
def callbackF(Xi):
    global Nfeval
    global theta1
    print ('{0:4d}     {1: 3.6f}    {2: 3.6f}     {3: 3.6f}     {4: 3.6f}'.format(Nfeval,  Xi[0], Xi[1], Xi[2],  MDbS(Xi,theta1,data_att_inn,y_choice)))
    Nfeval += 1

#####################generate choice########################################################
True_beta=np.array([ 0.642, -0.172, -1.751,  1.1  ])
                   #sc,beta_0,beta_1,asc_ev,theta
p_accum=cal_accumu_p(data_att_inn,True_beta)
boundary=math.ceil(True_beta[-1]*data_att_inn.shape[2])
...
tru_llh=MDbS(True_beta,True_beta[-1],data_att_inn,y_choice)        
 
#########################estimation################################################################
bnds1 = ((0.1,np.Inf),(-5,5),(-5,5))  #bound
Nfeval = 1
intial=np.array([-0.5,0.5,1])

theta1=1.1
print('Iteration  ','\u03B1           ','\u03B2_0         ','\u03B2_ 1      ','Log-Likelihood')
...
EST[1] = 0.7/(1+np.exp(-resOpt['x'][1]));EST[2] = -110/(1+np.exp(-resOpt['x'][2]))
ERR=(np.sqrt(np.diag(resOpt.hess_inv.todense()))).copy() #Standard erro
ERR[[1,2]]=trans(resOpt['x'],resOpt.hess_inv.todense())
...
\u03B2_0             {EST[1]:10.6f}   {ERR[1]:10.6f}     {Z_VAL[1]:10.6f}     {P_VAL[1]:10.6f} {SIG[1]}
\u03B2_ 1            {EST[2]:10.6f}   {ERR[2]:10.6f}     {Z_VAL[2]:10.6f}     {P_VAL[2]:10.6f} {SIG[2]}
------------------------------------------------------------------------------------
Significance:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

Log-Likelihood={-resOpt['fun']:10.4f}
BIC={len(True_beta)*np.log(N_id*N_scenarios)+2*resOpt['fun']:10.4f}
